...
def explain_with_shap(model, dataset: TensorDataset, anomaly_indices: list, 
    ...
    try:
        explainer = shap.GradientExplainer(model_wrapper, torch.from_numpy(x_background).float().to(device))
